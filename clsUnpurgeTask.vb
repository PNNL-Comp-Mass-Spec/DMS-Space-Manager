...
Namespace UnpurgeTask

  Public Interface IUnpurgeTaskParams
  ...
  Public Class clsUnpurgeTask
    Inherits clsDBTask
    Implements IUnpurgeTaskParams
...
#Region "parameters for calling stored procedures"
    ...
    Private mp_StorageVolExternal As String
		Private mp_ArchivePath As String
...
#End Region
    ...
    Public Function RequestTask(ByVal drive As String) As Boolean
      OpenConnection()
      RequestUnpurgeTask(drive)
      CLoseConnection()
      m_TaskWasAssigned = (mp_DatasetID <> 0)
      Return m_TaskWasAssigned
    End Function

    Public Sub CloseTask(ByVal closeOut As IUnpurgeTaskParams.CloseOutType, ByVal comment As String)
      OpenConnection()
      SetUnpurgeTaskComplete(GetCompletionCode(closeOut), comment)
      CLoseConnection()
    End Sub

    Private Function GetCompletionCode(ByVal closeOut As IUnpurgeTaskParams.CloseOutType) As Integer
      Dim code As Integer = 1      '  0->success, 1->failure, anything else ->no intermediate files
      Select Case closeOut
        Case IUnpurgeTaskParams.CloseOutType.CLOSEOUT_SUCCESS
          code = 0
        Case IUnpurgeTaskParams.CloseOutType.CLOSEOUT_FAILED
          code = 1
      End Select
      GetCompletionCode = code
    End Function

    '-------[for interface IUnpurgeTaskParams]----------------------------------------------
    Public Function GetParam(ByVal Name As String) As String Implements IUnpurgeTaskParams.GetParam
      Dim s As String
      Select Case (Name)
    ...
    Private Function RequestUnpurgeTask(ByVal drive As String) As Boolean
      ...
      Try
        ...
        '
        sc = New SqlCommand("RequestUnpurgeTask", m_DBCn)
        ...
        myParm = sc.Parameters.Add("@ArchivePath", SqlDbType.VarChar, 256)
        myParm.Direction = ParameterDirection.Output
        ...
        '
				...
				mp_ArchivePath = CStr(sc.Parameters("@ArchivePath").Value)
    ...
    Private Function SetUnpurgeTaskComplete(ByVal completionCode As Int32, ByRef message As String) As Boolean
      Dim sc As SqlCommand
      Dim Outcome As Boolean = False
      ...
      Try
        ...
        ' create the command object
        '
        sc = New SqlCommand("SetUnpurgeTaskComplete", m_DBCn)
        sc.CommandType = CommandType.StoredProcedure
